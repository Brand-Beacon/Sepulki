...
exports.taskResolvers = {
    Query: {
        async tasks(parent, args, context) {
            ...
            try {
                ...
                throw new errors_1.ServiceError('database', `Failed to fetch tasks: ${error}`);
            }
        },
        ...
        async runs(parent, args, context) {
            await (0, context_1.requirePermission)(context, shared_types_1.Permission.VIEW_TASKS);
            ...
            try {
                ...
                throw new errors_1.ServiceError('database', `Failed to fetch runs: ${error}`);
            }
        }
    },
    Mutation: {
        async dispatchTask(parent, { fleetId, input }, context) {
            const { smith } = await (0, context_1.requirePermission)(context, shared_types_1.Permission.CREATE_TASK);
            const { name, description, type, parameters, priority = shared_types_1.TaskPriority.NORMAL, scheduledAt } = input;
            // Validate input
            if (!name || name.trim().length === 0) {
                throw new errors_1.ValidationError('Task name is required', 'name');
            }
            if (!type) {
                throw new errors_1.ValidationError('Task type is required', 'type');
            }
            try {
                // Start transaction
          ...
          `;
                    ...
                    if (robotsResult.rows.length === 0) {
                        throw new errors_1.ValidationError('No available robots in fleet', 'fleetId');
                    }
                    ...
                    await client.query('INSERT INTO task_robots (task_id, robot_id, assigned_at) VALUES ($1, $2, NOW())', [task.id, assignedRobot.id]);
                    // Update task status
                    await client.query('UPDATE tasks SET status = $1, updated_at = NOW() WHERE id = $2', [shared_types_1.TaskStatus.ASSIGNED, task.id]);
                    // Update robot status if idle
                    if (assignedRobot.status === 'IDLE') {
                        await client.query('UPDATE robots SET status = $1, updated_at = NOW() WHERE id = $2', ['WORKING', assignedRobot.id]);
                    }
                    ...
                    return {
                        task: { ...task, status: shared_types_1.TaskStatus.ASSIGNED },
                        assignments: [{
                                taskId: task.id,
                                robotId: assignedRobot.id,
                                assignedAt: new Date(),
                ...
                throw new errors_1.ServiceError('database', `Failed to dispatch task: ${error}`);
            }
        },
        async cancelTask(parent, { taskId }, context) {
            await (0, context_1.requirePermission)(context, shared_types_1.Permission.CANCEL_TASK);
            try {
                // Update task status
                const result = await context.db.query(`UPDATE tasks 
           SET status = $1, updated_at = NOW() 
           WHERE id = $2 AND status IN ('PENDING', 'ASSIGNED', 'IN_PROGRESS')
           RETURNING *`, [shared_types_1.TaskStatus.CANCELLED, taskId]);
                if (result.rows.length === 0) {
                    throw new errors_1.NotFoundError('Task', taskId);
                }
                const task = result.rows[0];
                // Cancel any active runs
                await context.db.query(`UPDATE runs 
           SET status = 'CANCELLED', completed_at = NOW() 
           WHERE task_id = $1 AND status IN ('PENDING', 'RUNNING')`, [taskId]);
                // Free up assigned robots
                await context.db.query(`UPDATE robots 
           SET status = 'IDLE', updated_at = NOW() 
           WHERE id IN (
             SELECT robot_id FROM task_robots WHERE task_id = $1
           ) AND status = 'WORKING'`, [taskId]);
                // Clear cache
                ...
                throw new errors_1.ServiceError('database', `Failed to cancel task: ${error}`);
            }
        }
    },
    Subscription: {
    ...
    Task: {
        async assignedRobots(parent, args, context) {
            const result = await context.db.query(`SELECT r.* FROM robots r 
         JOIN task_robots tr ON r.id = tr.robot_id 
         WHERE tr.task_id = $1`, [parent.id]);
            return result.rows;
        },
        async runs(parent, args, context) {
            const result = await context.db.query('SELECT * FROM runs WHERE task_id = $1 ORDER BY started_at DESC', [parent.id]);
            return result.rows;
        },
