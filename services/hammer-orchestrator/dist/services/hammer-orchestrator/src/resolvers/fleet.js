...
function gpsToLocalCoordinates(gpsLat, gpsLng, locusLat, locusLng) {
...
exports.fleetResolvers = {
    Query: {
        async fleets(parent, args, context) {
            ...
            try {
                ...
                throw new errors_1.ServiceError('database', `Failed to fetch fleets: ${error}`);
            }
        },
        async fleet(parent, { id }, context) {
            await (0, context_1.requirePermission)(context, shared_types_1.Permission.VIEW_FLEET);
            const fleet = await context.dataloaders.fleet.load(id);
            if (!fleet) {
                throw new errors_1.NotFoundError('Fleet', id);
            }
            return fleet;
        },
        async robots(parent, args, context) {
            await (0, context_1.requirePermission)(context, shared_types_1.Permission.VIEW_ROBOTS);
            ...
            try {
                ...
                throw new errors_1.ServiceError('database', `Failed to fetch robots: ${error}`);
            }
        },
        async robot(parent, { id }, context) {
            await (0, context_1.requirePermission)(context, shared_types_1.Permission.VIEW_ROBOTS);
            const robot = await context.dataloaders.robot.load(id);
            if (!robot) {
                throw new errors_1.NotFoundError('Robot', id);
            }
            return robot;
        }
    },
    Mutation: {
        async updateRobotStatus(parent, { robotId, status }, context) {
            await (0, context_1.requirePermission)(context, shared_types_1.Permission.MANAGE_ROBOTS);
            if (!Object.values(shared_types_1.RobotStatus).includes(status)) {
                throw new errors_1.ValidationError(`Invalid robot status: ${status}`, 'status');
            }
            try {
                const result = await context.db.query(`UPDATE robots 
           SET status = $1, updated_at = NOW() 
           WHERE id = $2 
           RETURNING *`, [status, robotId]);
                ...
                throw new errors_1.ServiceError('database', `Failed to update robot status: ${error}`);
            }
        },
        async emergencyStop(parent, { fleetId }, context) {
            await (0, context_1.requirePermission)(context, shared_types_1.Permission.EMERGENCY_STOP);
            try {
           ...
           RETURNING *`, [fleetId]);
                // Update fleet status
                await context.db.query(`UPDATE fleets 
           SET status = 'MAINTENANCE', updated_at = NOW() 
           WHERE id = $1`, [fleetId]);
                // Cancel all active tasks for this fleet
                await context.db.query(`UPDATE tasks 
           ...
           WHERE id IN (
             ...
             WHERE r.fleet_id = $1 AND t.status IN ('PENDING', 'ASSIGNED', 'IN_PROGRESS')
                ...
                throw new errors_1.ServiceError('database', `Failed to execute emergency stop: ${error}`);
            }
        },
        async updateFleetLocation(parent, { fleetId, coordinates }, context) {
            ...
            try {
                ...
                const locusId = fleet.locus_id;
                if (!locusId) {
                    throw new errors_1.ValidationError('Fleet does not have a locus assigned', 'locus_id');
                }
                ...
                const result = await context.db.query(`UPDATE loci 
           SET coordinates = $1::jsonb, updated_at = NOW() 
           WHERE id = $2 
           RETURNING *`, [coordinatesJson, locusId]);
                if (result.rows.length === 0) {
                    throw new errors_1.NotFoundError('Locus', locusId);
                }
                // Clear caches
                context.dataloaders.fleet.clear(fleetId);
                ...
                throw new errors_1.ServiceError('database', `Failed to update fleet location: ${error}`);
            }
        },
        async updateRobotLocation(parent, { robotId, coordinates }, context) {
            await (0, context_1.requirePermission)(context, shared_types_1.Permission.MANAGE_FLEET);
            try {
                ...
                const locusResult = await context.db.query('SELECT coordinates FROM loci WHERE id = $1', [locusId]);
                if (locusResult.rows.length === 0 || !locusResult.rows[0].coordinates) {
                    throw new errors_1.ValidationError('Fleet locus does not have coordinates', 'locus');
                }
                ...
                const localCoords = gpsToLocalCoordinates(coordinates.latitude, coordinates.longitude, locusLat, locusLng);
                // Get current last_pose or create default
                const currentLastPose = robot.last_pose;
                const lastPose = {
                    position: {
                        x: localCoords.x,
                        y: localCoords.y,
           ...
           WHERE id = $2 
           RETURNING *`, [JSON.stringify(lastPose), robotId]);
                if (result.rows.length === 0) {
                    throw new errors_1.NotFoundError('Robot', robotId);
                }
                // Clear cache
                ...
                throw new errors_1.ServiceError('database', `Failed to update robot location: ${error}`);
            }
        }
    },
    Subscription: {
    ...
    Fleet: {
        async locus(parent, args, context) {
            if (!parent.locus_id)
                return null;
            const result = await context.db.query('SELECT * FROM loci WHERE id = $1', [parent.locus_id]);
            return result.rows[0] || null;
        },
        async robots(parent, args, context) {
            return await context.dataloaders.robotsByFleet.load(parent.id);
        },
        async activeTask(parent, args, context) {
            if (!parent.active_task_id)
                return null;
            return await context.dataloaders.task.load(parent.active_task_id);
        },
        async constraints(parent, args, context) {
            if (!parent.constraint_ids || parent.constraint_ids.length === 0)
                return [];
            const result = await context.db.query('SELECT * FROM edicts WHERE id = ANY($1)', [parent.constraint_ids]);
            return result.rows;
        },
        async telemetry(parent, args, context) {
    ...
    Robot: {
        ...
        fleetId: (parent) => parent.fleet_id,
        lastSeen: (parent) => parent.last_seen,
        batteryLevel: (parent) => parent.battery_level,
        healthScore: (parent) => parent.health_score,
        async currentIngot(parent, args, context) {
            if (!parent.current_ingot_id)
                return null;
            const result = await context.db.query('SELECT * FROM ingots WHERE id = $1', [parent.current_ingot_id]);
            return result.rows[0] || null;
        },
        async pose(parent, args, context) {
            ...
            if (position && (position.x !== undefined || position.y !== undefined || position.z !== undefined)) {
                ...
                if (fleet && fleet.locus_id) {
                    const locusQuery = 'SELECT coordinates FROM loci WHERE id = $1';
                    const locusResult = await context.db.query(locusQuery, [fleet.locus_id]);
                    if (locusResult.rows.length > 0 && locusResult.rows[0].coordinates) {
        ...
        async streamUrl(parent, args, context) {
        ...
        async factoryFloor(parent, args, context) {
            if (!parent.factory_floor_id)
                return null;
            try {
                ...
                throw new errors_1.ServiceError('database', `Failed to fetch factory floor for robot: ${error}`);
            }
        },
        async floorPositionX(parent, args, context) {
            return parent.floor_position_x ?? null;
        },
        async floorPositionY(parent, args, context) {
            return parent.floor_position_y ?? null;
        },
        async floorPositionTheta(parent, args, context) {
            return parent.floor_position_theta ?? null;
        },
        async isMobile(parent, args, context) {
        ...
        async floorRotationDegrees(parent, args, context) {
