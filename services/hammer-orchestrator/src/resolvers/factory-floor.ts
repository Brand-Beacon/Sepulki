...
export const factoryFloorResolvers = {
  Query: {
    async factoryFloors(parent: any, args: any, context: Context) {
      ...
      try {
        ...
        throw new ServiceError('database', `Failed to fetch factory floors: ${error}`);
      }
    },

    async factoryFloor(parent: any, { id }: { id: string }, context: Context) {
      await requirePermission(context, Permission.VIEW_FLEET);
      ...
      try {
        ...
        if (result.rows.length === 0) {
          throw new NotFoundError('FactoryFloor', id);
        }
        ...
        throw new ServiceError('database', `Failed to fetch factory floor: ${error}`);
      }
    },
  },

  Mutation: {
    async createFactoryFloor(parent: any, args: any, context: Context) {
      const { smith } = await requirePermission(context, Permission.MANAGE_FLEET);
      ...
      if (!name || name.trim().length === 0) {
        throw new ValidationError('Factory floor name is required', 'name');
      }

      if (!widthMeters || widthMeters <= 0) {
        throw new ValidationError('Width must be greater than 0', 'widthMeters');
      }

      if (!heightMeters || heightMeters <= 0) {
        throw new ValidationError('Height must be greater than 0', 'heightMeters');
      }

      if (!scaleFactor || scaleFactor <= 0) {
        throw new ValidationError('Scale factor must be greater than 0', 'scaleFactor');
      }

      if (!blueprintFile) {
        throw new ValidationError('Blueprint file is required', 'blueprintFile');
      }
      ...
      try {
        ...
        throw new ServiceError('database', `Failed to create factory floor: ${error}`);
      }
    },

    async updateFactoryFloor(parent: any, args: any, context: Context) {
      const { smith } = await requirePermission(context, Permission.MANAGE_FLEET);
      ...
      try {
        ...
        throw new ServiceError('database', `Failed to update factory floor: ${error}`);
      }
    },

    async deleteFactoryFloor(parent: any, { id }: { id: string }, context: Context) {
      ...
      try {
        ...
        throw new ServiceError('database', `Failed to delete factory floor: ${error}`);
      }
    },

    async assignRobotToFloor(parent: any, args: any, context: Context) {
      await requirePermission(context, Permission.MANAGE_FLEET);
      ...
      try {
        ...
        if (floorResult.rows.length === 0) {
          throw new NotFoundError('FactoryFloor', floorId);
          }
        }
        ...
        throw new ServiceError('database', `Failed to assign robot to floor: ${error}`);
      }
    },

    async updateRobotFloorPosition(parent: any, args: any, context: Context) {
      await requirePermission(context, Permission.MANAGE_FLEET);
      ...
      try {
        ...
        if (!robot.factory_floor_id) {
          throw new ValidationError('Robot is not assigned to a factory floor', 'robotId');
        }
        ...
        throw new ServiceError('database', `Failed to update robot floor position: ${error}`);
      }
    },

    async updateRobotMobility(parent: any, args: any, context: Context) {
      await requirePermission(context, Permission.MANAGE_FLEET);
      ...
      try {
        ...
        throw new ServiceError('database', `Failed to update robot mobility: ${error}`);
      }
    },
  },
  ...
  FactoryFloor: {
    ...
    async robots(parent: any, args: any, context: Context) {
      try {
        ...
        throw new ServiceError('database', `Failed to fetch robots for factory floor: ${error}`);
      }
    },
