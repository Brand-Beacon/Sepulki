...
import { NotFoundError, ValidationError, ServiceError, AuthorizationError } from '../errors';
import { Permission, EdictType, EdictSeverity } from '@sepulki/shared-types';
 ...
 */
function validateEdictRules(rules: any): void {
  if (!rules || typeof rules !== 'object') {
    throw new ValidationError('Rules must be a valid JSON object', 'rules');
  }

  // Check for required rule structure
  if (!rules.condition) {
    throw new ValidationError('Rules must include a condition object', 'rules.condition');
  }

  if (!rules.actions || !Array.isArray(rules.actions)) {
    throw new ValidationError('Rules must include an actions array', 'rules.actions');
  }
  ...
  if (!condition.type || !condition.operator || !Array.isArray(condition.rules)) {
    throw new ValidationError(
      'Condition must have type, operator, and rules array',
      'rules.condition'
    );
  }
  ...
  for (const action of rules.actions) {
    if (!action.type || !action.parameters || typeof action.immediate !== 'boolean') {
      throw new ValidationError(
        'Each action must have type, parameters, and immediate flag',
        'rules.actions'
      );
    }
  }
 ...
 */
function validateAppliesTo(appliesTo: any): void {
  if (!appliesTo || typeof appliesTo !== 'object') {
    throw new ValidationError('appliesTo must be a valid JSON object', 'appliesTo');
  }
  ...
  for (const field of arrayFields) {
    if (appliesTo[field] !== undefined && !Array.isArray(appliesTo[field])) {
      throw new ValidationError(
        `appliesTo.${field} must be an array`,
        `appliesTo.${field}`
      );
    }
  }
...
export const edictMutations = {
  ...
  async addEdict(
    ...
    const { smith } = await requirePermission(context, Permission.MANAGE_EDICTS);

    // Validate input
    if (!input.name || input.name.trim().length === 0) {
      throw new ValidationError('Edict name is required', 'name');
    }

    if (!Object.values(EdictType).includes(input.type)) {
      throw new ValidationError(
        `Invalid edict type: ${input.type}`,
        'type'
      );
    }

    if (!Object.values(EdictSeverity).includes(input.severity)) {
      throw new ValidationError(
        `Invalid severity level: ${input.severity}`,
        'severity'
      );
    }
    ...
    try {
      ...
      throw new ServiceError('database', `Failed to create edict: ${error}`);
    }
  },
  ...
  async updateEdict(
    ...
    const existingQuery = await context.db.query(
      'SELECT * FROM edicts WHERE id = $1',
      [id]
    );

    if (existingQuery.rows.length === 0) {
      throw new NotFoundError('Edict', id);
    }
    ...
    if (smith.role !== 'ADMIN' && existingEdict.created_by !== smith.id) {
      throw new AuthorizationError('You do not have permission to update this edict');
    }

    // Validate updated fields
    if (input.type && !Object.values(EdictType).includes(input.type)) {
      throw new ValidationError(`Invalid edict type: ${input.type}`, 'type');
    }

    if (input.severity && !Object.values(EdictSeverity).includes(input.severity)) {
      throw new ValidationError(`Invalid severity level: ${input.severity}`, 'severity');
    }
    ...
    try {
      ...
      const violationsQuery = await context.db.query(
        `SELECT COUNT(*) as count
         FROM policy_violations
         WHERE edict_id = $1 AND resolved = false`,
        [id]
      );
      ...
      throw new ServiceError('database', `Failed to update edict: ${error}`);
    }
  },
  ...
  async deactivateEdict(
    ...
    // Check permissions
    if (smith.role !== 'ADMIN' && existingEdict.created_by !== smith.id) {
      throw new AuthorizationError('You do not have permission to deactivate this edict');
    }
    ...
    try {
      ...
      throw new ServiceError('database', `Failed to deactivate edict: ${error}`);
    }
  },
  ...
  async recallFleet(
    parent: any,
    { fleetId, toVersion }: { fleetId: string; toVersion: string },
    context: Context
  ) {
    // Require RECALL_FLEET permission
    const { smith } = await requirePermission(context, Permission.RECALL_FLEET);
    ...
    try {
      ...
      try {
        ...
        const robotsQuery = await client.query(
          'SELECT id, sepulka_id, current_ingot_id FROM robots WHERE fleet_id = $1 LIMIT 1',
          [fleetId]
        );

        if (robotsQuery.rows.length === 0) {
          throw new ValidationError('Fleet has no robots', 'fleetId');
        }
        ...
        const ingotQuery = await client.query(
        ...
        if (ingotQuery.rows.length === 0) {
          throw new NotFoundError('Ingot version', toVersion);
        }
        ...
        if (!['READY', 'TEMPERED', 'DEPLOYED'].includes(targetIngot.status)) {
          throw new ValidationError(
            `Cannot recall to ingot with status: ${targetIngot.status}`,
            'toVersion'
          );
        }

        // Determine if this is a rollback by checking if robots currently have a different version
        const currentVersionQuery = await client.query(
        ...
        const allRobotsQuery = await client.query(
          'SELECT id, name, sepulka_id FROM robots WHERE fleet_id = $1',
          [fleetId]
        );

        const robots = allRobotsQuery.rows;
        const successfulUpdates: string[] = [];
        const failedUpdates: Array<{ robotId: string; error: string }> = [];
        ...
        for (const robot of robots) {
          try {
            ...
            if (robot.sepulka_id !== sepulkaId) {
              const robotIngotQuery = await client.query(
                `SELECT id FROM ingots
                 WHERE sepulka_id = $1 AND version = $2
                 ORDER BY created_at DESC
                 LIMIT 1`,
                [robot.sepulka_id, toVersion]
              );
      ...
      throw new ServiceError('database', `Failed to recall fleet: ${error}`);
    }
  }
};
