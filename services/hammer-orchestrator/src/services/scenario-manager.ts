...
export class ScenarioManager {
  ...
  private activeScenarios: Map<string, ScenarioConfig> = new Map();

  // Pre-defined base locations for demos
  private readonly BASE_LOCATIONS = {
  ...
  constructor(generator: TelemetryGenerator) {
   ...
   */
  public initializeLawnMowingScenario(fleetId: string, robotIds: string[]): ScenarioConfig {
    ...
    robotIds.forEach((robotId, index) => {
      ...
      const zoneTopLeft: GPSCoordinate = {
      ...
      const path = this.generator.generateGridPath(
        zoneTopLeft,
        zoneWidth,
        zoneHeight,
        2 // 2 meter row spacing
      );

      // Initialize robot at first waypoint
      this.generator.initializeRobot(
        robotId,
        fleetId,
        path[0],
        RobotStatus.WORKING
      ...
      if (state) {
        state.activity = RobotActivity.WORKING;
        state.speed = 0.8;
      }
    });

    const config: ScenarioConfig = {
      type: ScenarioType.LAWN_MOWING,
   ...
   */
  public initializeWarehouseScenario(fleetId: string, robotIds: string[]): ScenarioConfig {
    ...
    robotIds.forEach((robotId, index) => {
      ...
      let startZone: typeof zones.receiving;
      let endZone: typeof zones.storage;
      let speed: number;
      ...
      const startPoint: GPSCoordinate = {
    ...
    const config: ScenarioConfig = {
      type: ScenarioType.WAREHOUSE_LOGISTICS,
   ...
   */
  public initializeAgricultureScenario(fleetId: string, robotIds: string[]): ScenarioConfig {
   ...
   */
  public initializeCustomScenario(
    fleetId: string,
    robotIds: string[],
   ...
   */
  public getScenario(fleetId: string): ScenarioConfig | undefined {
   ...
   */
  public listScenarios(): ScenarioConfig[] {
   ...
   */
  public stopScenario(fleetId: string): void {
   ...
   */
  private generateWarehouseRoute(
    start: GPSCoordinate,
    end: GPSCoordinate,
    intermediatePoints: number
  ): GPSCoordinate[] {
    const waypoints: GPSCoordinate[] = [start];
   ...
   */
  public getRecommendedScenario(robotCount: number): ScenarioType {
    if (robotCount <= 8) {
  ...
  public simulateEvent(
    fleetId: string,
    eventType: 'task_complete' | 'battery_swap' | 'maintenance' | 'emergency'
  ): void {
