...
app.use(express_1.default.json({ limit: '10kb' })); // Strict size limit for auth service
...
app.get('/auth/csrf', security_1.csrfTokenEndpoint);
// Login endpoint (mimics Auth.js callback) with strict rate limiting
app.post('/auth/signin', security_1.loginRateLimiter, async (req, res) => {
    const { email, password, callbackUrl } = req.body;
    try {
        // Find user in database (same as production)
        const userQuery = await db.query('SELECT id, email, name, password_hash, role, permissions FROM smiths WHERE email = $1 AND is_active = true', [email.toLowerCase()]);
        ...
        await db.query('UPDATE smiths SET last_login_at = NOW() WHERE id = $1', [user.id]);
