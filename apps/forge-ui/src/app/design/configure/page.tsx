...
import { FORGE_SEPULKA_MUTATION } from '@/lib/graphql/mutations';
...
function ConfigureContent() {
  ...
  const [forgeSepulka, { loading: forging }] = useMutation(FORGE_SEPULKA_MUTATION);
  ...
  const handleDeployToFleet = async () => {
    if (!selectedRobot || !smith) {
      alert('Please select a robot and sign in to deploy');
      return;
    }

    try {
      // First, create the robot design (sepulka) from the recommended robot
      const result = await forgeSepulka({
        variables: {
          input: {
            name: `${selectedRobot.name} - From RAG Recommendation`,
            description: `Deployed from RAG analysis: ${userInput}`,
            patternId: patternId,
      ...
      if (forgeData?.forgeSepulka?.sepulka?.id) {
        ...
        // Redirect to fleet deployment page or show deployment modal
        router.push(`/fleet?deploy=${sepulkaId}`);
      }
    } catch (error) {
      console.error('Failed to create robot design:', error);
      alert('Failed to create robot design. Please try again.');
    }
  };
                ...
                {selectedRobot && smith && (
                  <div className="mt-4 pt-4 border-t">
                    ...
                    >
                      {forging ? (
                        <>
                          ...
                          <span>Creating Robot Design...</span>
                        </>
                      ) : (
                        <>
                    ...
                    <p className="text-xs text-gray-500 mt-2 text-center">
                      Create robot from recommendation and deploy to your fleet
                    </p>
                  </div>
                )}
              </div>
      ...
      <div className="flex justify-between space-x-4 mt-8">
        ...
        <div className="flex space-x-3">
          ...
          {selectedRobot && (
            <button
              onClick={() => {
                const designId = Date.now().toString(); // Temporary ID until saved
                localStorage.setItem('currentDesignId', designId);
                router.push(`/design/${designId}/review`);
              }}
